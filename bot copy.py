...
async def sygn(ctx, caseType, caseName=" ", region="east"):
    ...
    if isAllowedChannel:
        ...
        match region:
            case 'east':
                chid = os.getenv("CHANNEL_EAST")
            case 'west':
                chid = os.getenv("CHANNEL_WEST")
            
        channel = bot.get_channel(chid) # define this!
        await channel.create_thread(name=result, message=f"{result}; requested by: {usr.mention}", auto_archive_duration=60, type=None, reason=None)
    else:
        await ctx.send(f"You are not allowed to use this command here.")
